...
# Purpose:     Adding Logging to Program
#
# Author:      Siddharth Nema
# Created:     08-June-2021
# Updated:     08-June-2021
#-----------------------------------------------------------------------------
# Import Logging and Log to a File
import logging
...
def milesToKm(distance):
  ...
  '''
    logging.info(f"milestoKm()function beginning with {distance} Miles.")
    # Checks is Parameters are Correct Type, Raises TypeError
    if not isinstance(distance, (int,float)):
      logging.error(f"Invalid Paramter Type for Distance")
      raise TypeError("Invalid Parameter Type")
    # Checks if Parameters are Correct Values, Raises ValueError
    if distance < 0:
        logging.error(f"The Distance is Negative or Invalid.")
    ...
    # Otherwise, Proceeds with Calculation & Returns Result to 2 Decimal Places
        ...
        logging.info(f"The Conoverted Distance in Kilometres is {conversion} Km.")
        return conversion
...
def saleCalc(price, discount):
  ...
  '''
    logging.info(f"saleCalc() Function beginning with Item Price of ${price} & a {discount}% Discount")
    # Checks is Parameters are Correct Type, Raises TypeError
    if not isinstance(price,(int, float)) or not isinstance(discount,int):
      logging.error(f"One of the paramters is not the correct type.")
      raise TypeError("Incorrect Paramter Type")

    # Checks if Parameters are Correct Values, Raises ValueError
    if price < 0 or discount < 0 or discount >= 100:
      logging.error(f"One of the paramters is negative or the discount exceeds 100")
      raise ValueError("Discount & Price must be a Positive Value.")
    # Otherwise, Proceeds with Calculation & Returns Result to 2 Decimal Places
    else:
        finalPrice = round(price * (1-(discount/100)), 2)
        logging.info(f"The Final Price of the item is ${finalPrice}")
        return finalPrice
...
while bool(storeOneKm) == False:
    try:
        ...
        logging.debug(f"Gathering the Store One's Information.")
        ...
        logging.info(f"Calling saleCalc() Function with Item Price of ${storeOnePrice} & {storeOneDiscount}% Discount.")
        storeOneFinal = saleCalc(storeOnePrice, storeOneDiscount)
        logging.info(f"Calling milesToKm() Function with Distance of {storeOneMiles} Miles.")
...
while bool(storeTwoKm) == False:
    ...
    # Enter Price, Discount & Distance for Store Two
        logging.debug(f"Gathering the Store Two's Information.")
        ...
        logging.info(f"Calling saleCalc() Function with Item Price of ${storeTwoPrice} & {storeTwoDiscount}% Discount.")
        storeTwoFinal = saleCalc(storeTwoPrice, storeTwoDiscount)
        logging.info(f"Calling milesToKm() Function with Distance of {storeTwoMiles} Miles.")
...
logging.debug(f"Information Gathered, Proceeding with To Compare Store Price and Distance.")
...
if storeOneFinal > storeTwoFinal:
    ...
    logging.info(f"Store One is cheaper by ${priceDiff}, buy the item from Store One")
    ...
    logging.info(f"Store Two is cheaper by ${priceDiff}, buy the item from Store Two")
    ...
    if storeOneKm > storeTwoKm:
        ...
        logging.info(f"Both Store's have the Same Price, But storeOneKm is closer by {distDiff} Km.")
        print(f"Both Store's have the Same Price, But storeOneKm is closer by {distDiff} Km.")
    elif storeOneKm > storeTwoKm:
        distDiff = storeOneKm - storeTwoKm
        logging.info(f"Both Store's have the Same Price, But storeTwoKm is closer by {distDiff} Km.")
        print(f"Both Store's have the Same Price, But storeTwoKm is closer by {distDiff} Km.")
    else:
      logging.info("Both Store's have the Same Price & are Equally Far from your Home!")
